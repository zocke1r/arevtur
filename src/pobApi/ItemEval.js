...
class ItemEval extends CustomOsScript {
	...
	spawnProcess(pobPath) {
		return spawn(path.resolve(path.join(__dirname, "./luajit.exe")), [path.join(__dirname, "./itemEcho.lua")], { cwd: path.resolve(pobPath) });
	}
	...
	evalItem(item) {
		...
		return new Promise(r => this.pendingResponses.push(r)).then(text => ItemEval.clean(text));
	}
	...
	evalItemModSummary(itemMod, pluginNumber = 1, raw = false) {
		...
		return new Promise(r => this.pendingResponses.push(r))
			...
			.then(text => {
				...
				let resist = text.match(new RegExp(resistRegex, "gi"))?.reduce((sum, m) => sum + Number(m.match(resistRegex)[1]), 0) || 0;
				let value =
					Math.round(
						((((dps * 80) / 3) * (this.valueParams_?.dps || 1) + life * (this.valueParams_?.life || 1) + (resist / 3) * (this.valueParams_?.resist || 1)) /
							pluginNumber) *
							100
					) / 100;
	...
	static clean(outString) {
		console.log(`dirty string:${outString}`);
		return outString
			.replace(/\^x[\dA-F]{6}/g, "")
			.replace(/\^\d/g, "")
			.replace(/\r/g, "")
